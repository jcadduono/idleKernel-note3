...
static int sec_therm_get_adc_data(struct sec_therm_info *info)
{
	...
	for (i = 0; i < ADC_SAMPLING_CNT; i++) {
		...
		rc = qpnp_vadc_read(NULL, LR_MUX4_PU2_AMUX_THM1 , &results);

		if (rc) {
			dev_err(info->dev, "error reading thermal data [rc = %d]\n", rc);
			goto err;
		}
		...
		if (i == 0) {
			dev_info(info->dev,
				"sampling thermal data [adc_code = %d]\n", adc_data);
			adc_max = adc_data;
			adc_min = adc_data;
		}
...
static __devinit int sec_therm_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "loading thermal probe module\n");
...
{
	pr_info("%s: initializing thermistor driver\n", KBUILD_MODNAME);
