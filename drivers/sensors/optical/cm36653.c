...
static int cm36653_parse_dt(struct device *dev,
			struct  cm36653_platform_data *pdata)
{
	...
	pdata->irq = of_get_named_gpio_flags(np, "capella,irq_gpio",
			     ...
			     const struct i2c_device_id *id)
{
	...
	struct cm36653_platform_data *pdata= NULL;
	u16 threshold = 0;
    ...
    if(client->dev.of_node) {
		pdata = devm_kzalloc (&client->dev ,
			sizeof(struct cm36653_platform_data ), GFP_KERNEL);
		if(!pdata) {
		dev_err(&client->dev, "Failed to allocate memory\n");
		return -ENOMEM;
		}
		err = cm36653_parse_dt(&client->dev, pdata);
		if(err)
			goto err_devicetree;
	} else
...
#else
#define cm36653_match_table NULL
#endif
